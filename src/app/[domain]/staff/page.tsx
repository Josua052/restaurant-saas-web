...
export default async function TenantStaffDashboard({
  ...
  const getStatusBadge = (status: number) => {
    switch (status) {
      case 2: // Confirmed
        return <span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-xs font-semibold">Confirmed</span>;
      case 3: // CheckedIn/Arrived
        return <span className="bg-[#EAEBFE] text-indigo-700 px-3 py-1 rounded-full text-xs font-semibold">Arrived</span>;
      case 4: // Seated
        return <span className="bg-[#D1F4E0] text-emerald-700 px-3 py-1 rounded-full text-xs font-semibold">Seated</span>;
      default: // Pending / Others
        return <span className="bg-slate-100 text-slate-700 px-3 py-1 rounded-full text-xs font-semibold">Pending</span>;
    }
  };
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden flex flex-col justify-between h-[180px]">
          ...
          <div className="flex justify-between items-start relative z-10">
            ...
            <div className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-xs font-bold">
              {stats?.occupied_tables || 0}/{stats?.total_tables || 0} Capacity
            </div>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {stats?.upcoming_reservations && stats.upcoming_reservations.length > 0 ? (
                  ...
                  <tr key={idx} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-slate-900">{res.guest_name}</span>
                      </div>
                    </td>
