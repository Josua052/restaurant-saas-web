...
export default async function ReservationsPage() {
  ...
  let ownerName = "";
  let reservations = [];
  let reservationStats = { total_reservations: 0, upcoming_reservations: [], pending_confirmations: 0 };
  let tableStats = { total_tables: 0, active_tables: 0 };
  let menuStats = { active_menu_items: 0, sold_out_items: 0, categories_count: 0 };
  ...
  if (token) {
    try {
      ...
      const meRes = await fetch(`${API_URL}/management/auth/me`, { headers, cache: "no-store" });
      if (meRes.ok) {
      ...
      const [resList, resStats, tabStats, menStats] = await Promise.all([
        fetch(`${API_URL}/management/reservations`, { headers, cache: "no-store" }),
