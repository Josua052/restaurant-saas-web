...
export default function ProfileClient() {
  ...
  const isPasswordFormFilled = currentPassword.length > 0 && newPassword.length > 0 && confirmPassword.length > 0
  const isPasswordsMatch = newPassword === confirmPassword || confirmPassword.length === 0

  return (
    <div suppressHydrationWarning className="w-full max-w-4xl space-y-8 pb-10">
      
      {/* Page Header */}
      <div>
        <h1 className="text-[28px] font-bold text-slate-900 font-heading tracking-tight">My Profile</h1>
        <p className="text-slate-500 mt-1 text-sm md:text-base">View and manage your account information.</p>
      </div>
              ...
              <div className="sm:col-span-2">
                ...
                {!isEditing ? (
                  <div className="flex items-center gap-2">
                    <p className="text-slate-900 font-medium text-[15px]">{personalData.email}</p>
                ...
                <p className="mt-2 text-xs text-slate-500">
                  This email is locked to protect system access. Contact your technical team to change it.
                </p>
              </div>
        ...
        <div className="p-5 md:p-6 lg:p-8">
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-500 mt-1">
              Update your password to keep your account secure. This account has full server access — choose a strong password.
            </p>
          </div>

          <form onSubmit={handleChangePassword} className="border-t border-slate-100 pt-6 space-y-6">
            ...
            <div className="max-w-xl">
              ...
              {!isPasswordsMatch && (
                <p className="mt-2 text-xs font-medium text-red-500 flex items-center gap-1.5">
                  <span className="inline-flex items-center justify-center w-3 h-3 rounded-full border border-red-500 text-[8px] leading-none">!</span>
